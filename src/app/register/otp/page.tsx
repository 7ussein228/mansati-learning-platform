...
import { Phone, Lock, User as UserIcon, Loader2, GraduationCap, ShieldCheck, ArrowRight } from 'lucide-react';
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('/api/auth/otp-register', {
        ...
        body: JSON.stringify({ idToken, phone: firebaseUser.phoneNumber, name: name.trim() }),
      });
      ...
      <div className="hidden lg:flex flex-col justify-center p-12 bg-gradient-to-bl from-blue-600 to-indigo-800 text-white relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <p className="text-blue-100 text-lg mb-8 leading-relaxed">
            سجّل في ثوانٍ usando رقم موبايلك فقط، بدون إيميل أو كلمة مرور.
          </p>
          <div className="space-y-4">
            {[
              'سجّل في ثوانٍ معدودة',
              'أمان بفضل Firebase Authentication',
              'ادخل بموبايلك في أي وقت',
            ].map((t, i) => (
        ...
        <div className="w-full max-w-md">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
            <h2 className="text-2xl font-extrabold text-slate-900 mb-2">
              {step === 'info' ? 'إنشاء حساب جديد' : 'كود التحقق'}
            </h2>
            <p className="text-slate-500 text-sm mb-6">
              {step === 'info'
                ? 'أدخل بياناتك ثم رقم الموبايل لاستلام كود التحقق'
            ...
            <div className="mt-6 text-center text-sm text-slate-600">
              لديك حساب بالفعل؟{' '}
              <Link href="/login/otp" className="text-blue-600 hover:text-blue-700 font-bold">
                سجّل الدخول
              </Link>
            </div>

            <div className="mt-4 pt-4 border-t border-slate-100 text-center">
              <Link href="/register" className="text-sm text-slate-500 hover:text-blue-600 transition font-medium">
